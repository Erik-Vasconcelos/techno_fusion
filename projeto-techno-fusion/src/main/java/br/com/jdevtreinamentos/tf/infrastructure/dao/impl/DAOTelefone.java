...
public class DAOTelefone implements Serializable, EntidadeGenericaDAO<Telefone> {
	...
	public Set<Telefone> salvar(Set<Telefone> telefones) {
		removerTelefonesExcluidos(telefones);
	...
	public void removerTelefonesExcluidos(Set<Telefone> novosTelefones) {
		Long idFuncionario = new ArrayList<Telefone>(novosTelefones).get(0).getFuncionario().getId();
		
		Set<Telefone> telefonesAntigos = new HashSet<Telefone>(buscarTelefonesPorFuncionario(idFuncionario));
		
		Set<Telefone> telefonesRemover = telefonesAntigos.stream().filter(t -> !novosTelefones.contains(t)).collect(Collectors.toSet());
		
		telefonesRemover.stream().forEach(t -> {System.out.println(t.getNumero()); excluirPorId(t.getId());});
	}
